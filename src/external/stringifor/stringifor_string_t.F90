...
public :: lgt, lge, llt, lle, char, ichar, iachar, count
...
type, extends(string_class) :: string
  ...
  contains
    ...
    ! operators
    generic :: assignment(=) => string_assign_integer_I1P, &
    ...
    ! logical operators
    ! IO
    procedure, private, pass(self) :: read_delimited                !< Read a delimited input.
    procedure, private, pass(self) :: read_undelimited              !< Read an undelimited input.
    procedure, private, pass(self) :: read_undelimited_listdirected !< Read an undelimited list directed input.
    ! miscellanea
...
contains
    ...
    elemental function new_string(string_) result(new)
  ...
  pure function strjoin_characters_array(array, sep, is_trim, is_col) result(join)
   ...
   class(string),             intent(out)   :: self       !< The string.
   integer,                   intent(in)    :: unit      !< Logical unit.
  ...
  subroutine read_undelimited_listdirected(self, unit, iostat, iomsg)
  ...
  if (iostat /= 0) return
  call self%read_undelimited(unit=unit, terminators=' '//'/'//merge(CK_',', CK_';', decimal_point), iostat=iostat, iomsg=iomsg)
  endsubroutine read_undelimited_listdirected

  subroutine read_undelimited(self, unit, terminators, iostat, iomsg)
